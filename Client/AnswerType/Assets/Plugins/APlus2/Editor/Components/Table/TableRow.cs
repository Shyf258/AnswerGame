...
namespace APlus2
{
    public class TableRow : Element
    {
        ...
        public TableRow(List<Column> columns)
        {
            ...
            {
                var control = item.GetColumnAction().CreateCellControl();
        ...
        public void SetData(int index, APAsset data)
        {
            ...
            this.columns[0].GetColumnAction().SetCellData(this.controls[0], index);
            for (int i = 1; i < controls.Count; i++)
            {
                this.columns[i].GetColumnAction().SetCellData(this.controls[i], data);
