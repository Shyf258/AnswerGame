...
public class Editor_Log
{
    private class LogEditorConfig
    {
        public string _logScriptPath = "";   //自定义日志脚本路径
        public string _logTypeName = "";     //脚本type
        public int _instanceID = 0;

        public LogEditorConfig(string logScriptPath, System.Type logType)
        {
            this._logScriptPath = logScriptPath;
            this._logTypeName = logType.FullName;
        }
    }
    ...
    {
        new LogEditorConfig("Assets/Scripts/Tools/DDebug.cs",typeof(DDebug))
    };
    ...
    private static void UpdateLogInstanceID(LogEditorConfig config)
    {
        ...
        var assetLoadTmp = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(config._logScriptPath);
        if (null == assetLoadTmp)
        {
            throw new System.Exception("not find asset by path=" + config._logScriptPath);
        }
    ...
    private static string GetCurrentFullFileName(string[] fileNames)
    {
        ...
        for (int i = fileNames.Length - 1; i >= 0; --i)
        {
            ...
            {
                if (fileNames[i].Contains(_logEditorConfig[j]._logTypeName))
                {
                    isCustomLog = true;
                    break;
                }
